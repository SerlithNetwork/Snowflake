...
record PatchEntry(
    ...
    void applyPatch(final Map<String, Map<String, URL>> urls, final Path originalRootDir, final Path repoDir, final Path versionDir) throws IOException {
        final Path inputDir = originalRootDir.resolve("META-INF").resolve(this.location);
        final Path targetDir = this.location.equalsIgnoreCase("versions") ? versionDir.resolve(this.location) : repoDir.resolve(this.location);
